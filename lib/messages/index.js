...
export const karma = {
  ...
  voter_increased_karma_to_target: function({ voter, voterKarma, target, targetNewKarma }) {
    return userFullName(voter, true) + ' (' + voterKarma + ') дал рофлан ' + userFullName(target, true) + ' ('+ targetNewKarma +')';
  },
  voter_decreased_karma_to_target: function({ voter, voterKarma, target, targetNewKarma }) {
    return userFullName(voter, true) + ' (' + voterKarma + ') залил соляры ' + userFullName(target, true) + ' ('+ targetNewKarma +')';
  },
...
export const service = {
  ...
  someone_joined_group: function ({ user }) {
    return 'Добрый почантек, ' + userFullName(user, true) + '!';
  }
};
