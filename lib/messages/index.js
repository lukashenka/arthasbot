...
export const karma = {
  ...
  voter_increased_karma_to_target: function({ voter, voterKarma, target, targetNewKarma }) {
    return userFullName(voter, true) + ' (' + voterKarma + ') плюсанул в карму ' + userFullName(target, true) + ' ('+ targetNewKarma +')';
  },
  voter_decreased_karma_to_target: function({ voter, voterKarma, target, targetNewKarma }) {
    return userFullName(voter, true) + ' (' + voterKarma + ') насрал в карму ' + userFullName(target, true) + ' ('+ targetNewKarma +')';
  },
  voter_try_to_change_bot_karma: function () {
    return 'У меня нет кармы 😖😫';
  },
  target_have_karma_ban: function ({ user }) {
    return 'У ' + userFullName(user, true) + ' отключена карма в этом чате';
  },
...
export const service = {
  ...
  someone_joined_group: function ({ user }) {
    return 'Привет, ' + userFullName(user, true) + '! Какой рейт на Upwork? Какой дошик любишь?';
  }
};
